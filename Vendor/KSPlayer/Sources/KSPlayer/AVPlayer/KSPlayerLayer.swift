    ...
    public var player: MediaPlayerProtocol {
        didSet {
            ...
            runOnMainThread { [weak self] in
                ...
                // its renderSource (the MEPlayerItem with every cache), so on
                // an ENGINE SWAP the old pipeline could never deinit and the
                // whole chain leaked. This player is being REPLACED, never
                // reused, so killing its render loop here is always safe.
                (oldValue as? KSMEPlayer)?.videoOutput?.invalidate()
            }
