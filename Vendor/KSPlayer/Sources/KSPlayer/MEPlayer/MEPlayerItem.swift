    ...
    private func readThread() {
        ...
        while [MESourceState.paused, .seeking, .reading].contains(state) {
            if state == .paused {
                ...
                // depends on readOperation, that wedged the ENTIRE teardown:
                // format contexts, packet queues and frame queues of a dead
                // player stayed allocated for the rest of the process's life
                // (~400-500 MB per engine swap, measured live on tvOS via
                // task_vm_info: the pages migrate to the compressor at the
                // swap and never free). A bounded wait converts the missed
                // wakeup from a permanent leak into at most 500 ms of lag.
                condition.wait(until: Date() + 0.5)
            }
            if state == .seeking {
