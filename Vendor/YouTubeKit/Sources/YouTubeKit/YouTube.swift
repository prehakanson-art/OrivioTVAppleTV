...
public class YouTube {
    ...
    public var streams: [Stream] {
        get async throws {
            ...
            // the whole call with it, including `.remote`, which does not need
            // that page at all. The page is unreadable exactly when YouTube
            // answers with a consent wall or a "confirm you're not a bot"
            // interstitial instead of the video, which is what an address with
            // a poor reputation gets: a VPN exit, shared by however many other
            // people sit behind it, collects those constantly. The result was
            // that turning a VPN on took every trailer away, with the one
            // extractor that could still have delivered never given its turn.
            //
            // Only `regexMatchError` — "I could not read this page" — is
            // swallowed. A page we CAN read that says private, members-only or
            // unavailable still throws, so a genuinely unplayable video is
            // still reported as one rather than being dragged through a
            // pointless extraction.
            do {
                try await checkAvailability()
            } catch YouTubeKitError.regexMatchError {
                os_log("Couldn't read the watch page for availability — continuing to the extraction methods", log: log, type: .info)
            }
