...
struct OrivioCollectionFolder: Codable, Identifiable, Hashable {
    var id: String
    var title: String
...
struct OrivioCollection: Codable, Identifiable, Hashable {
    ...
    var folders: [OrivioCollectionFolder] = []
    ...
    init(from decoder: Decoder) throws {
        ...
        folders = (try c.decodeIfPresent([Lenient<OrivioCollectionFolder>].self, forKey: .folders) ?? []).compactMap(\.value)
    }

    init(id: String, title: String, folders: [OrivioCollectionFolder] = []) {
        self.id = id
        self.title = title
...
final class CollectionsStore: ObservableObject {
    ...
    @Published private(set) var collections: [OrivioCollection] = []
    ...
    @Published private(set) var library: [OrivioCollection] = []
    ...
    private static let baseKey = "orivio.collections.v1"
    /// Account-wide library key (no profile suffix).
    private static let libraryKey = "orivio.collections.library.v1"
    ...
    nonisolated static func readPersistedLibrary() -> [OrivioCollection]? {
        if let data = try? Data(contentsOf: libraryFileURL),
           let decoded = try? JSONDecoder().decode([OrivioCollection].self, from: data) {
            return decoded
        }
    ...
    func update(_ collection: OrivioCollection) {
    ...
    func applyRemote(json: String) -> Bool {
        ...
        guard let lenient = try? JSONDecoder().decode([Lenient<OrivioCollection>].self, from: data) else { return false }
    ...
    @discardableResult
    func applyRemote(collections remote: [OrivioCollection]) -> Bool {
    ...
    nonisolated private static func migrateLegacyProfileCollections() -> [OrivioCollection] {
        var byTitle: [String: OrivioCollection] = [:]
        var order: [String] = []
        for pid in 1...12 {
    ...
    nonisolated private static func richness(_ c: OrivioCollection) -> Int {
